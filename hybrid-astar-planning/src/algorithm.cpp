...
bool Compare3d::operator()(const State &s1, const State &s2) {
  ...
  //return s1.cost3d + Algorithm::holonomicWithObs(s1) + Algorithm::nonHolonomicWithoutObs(s1) >
         //s2.cost3d + Algorithm::holonomicWithObs(s2) + Algorithm::nonHolonomicWithoutObs(s1);
}
...
void Algorithm::hybridAstarPlanning() {
  ...
  // Run the A* Search Algorithm on the 2d grid map.
  astarPlanning();

  m_map.initCollisionChecker();
  m_map.findNearObs();
  int WALL_FOLLOW_STEP_SIZE = VEH_LEN/2;
  int MAX_STEPS = 50;
  int WALL_FOLLOW_TURN_ANGLE = 10;
  ...
  while (pq.size() > 0) {
    ...
    // 检测碰撞
    /*if (m_map.checkCollision(current)) {
        // 墙跟随逻辑
        int steps = 0; // 防止无限循环
        while (m_map.checkCollision(current) && steps < MAX_STEPS) {
            // 调整方向
            current.theta += WALL_FOLLOW_TURN_ANGLE;
            if (current.theta >= 360) current.theta -= 360;

            // 向前移动
            current.x += cos(current.theta * PI / 180) * WALL_FOLLOW_STEP_SIZE;
            current.y += sin(current.theta * PI / 180) * WALL_FOLLOW_STEP_SIZE;

            // 检查是否能重新进入A*路径
            if (!m_map.checkCollision(current)) {
                pq.push(current); // 重新加入优先队列继续A*搜索
                break;
            }
            steps++;
        }
    ...
    /*if (m_map.isParallelAndNearby(current, m_map, VEH_LEN, edgeDirection)) {
        // 沿障碍物边缘行驶
        // 计算当前位置到目标的方向
      Point toGoal = {Algorithm::goal.x - current.x, Algorithm::goal.y - current.y};
      double norm = sqrt(toGoal.x * toGoal.x + toGoal.y * toGoal.y);
      toGoal.x /= norm;
      toGoal.y /= norm;

      // 沿障碍物边缘行驶，同时确保方向接近目标
      while (m_map.isParallelAndNearby(current, m_map, VEH_LEN, edgeDirection)) 
       {
          double result = edgeDirection.x * toGoal.x + edgeDirection.y * toGoal.y;
          double sign = std::copysign(1.0, result); 
          // 更新小车位置，沿着 edgeDirection 移动
          double len = sqrt(edgeDirection.x * edgeDirection.x + edgeDirection.y * edgeDirection.y);
          edgeDirection.x /= len;
          edgeDirection.y /= len;

          current.x += sign * edgeDirection.x * VEH_LEN;
          current.y += sign * edgeDirection.y * VEH_LEN;

          // 显示小车位置
          display.drawCar(current);
          display.show(2000 / current.velocity);
      }
    } */
    if (abs(current.gx - goal.gx) <= 1 && abs(current.gy - goal.gy) <= 1 &&
      ...
      // Draw trajectory from initial to goal.
    ...
    std::cout << "Path search time: " << duration.count() << " milliseconds" << std::endl;
    double totalPathLength = 0;
    std::ofstream outFile("/home/jiamingyu/Hybrid_A_Star/Hybrid_A_Star-master/hybrid-astar-planning/data/path.txt");
    if (outFile.is_open()) {
    // display the path in reversed order
      for (int i = path.size() - 1; i >= 0; i--) {
        if (i > 0) {
        double stepLength = std::sqrt(std::pow(path[i].x - path[i-1].x, 2) + std::pow(path[i].y - path[i-1].y, 2));
        totalPathLength += stepLength;
    }

        // Display the car in the GUI
        display.drawCar(path[i]);
        display.show(2000 / path[i].velocity);

        // Write the position and orientation (theta) of the car to the file
        outFile << "(" << path[i].x << ", " << path[i].y << ", " << path[i].movementDirection << ")" << std::endl;
    }
      std::cout << "Total Path Length: " << totalPathLength << std::endl;
      outFile << "Total Path Length: " << totalPathLength << std::endl;
      outFile.close(); // Close the file after writing all the points
    } else {
        std::cerr << "Unable to open file for writing the path." << std::endl;
    }
      break;
    }
    ...
    std::vector<State> next = current.getNextStates(m_map);
